...
Z_critical <-  qnorm(1 - alpha) ; round(Z_critical,2)
z_curves(two_sided = "greater")
paste0("If ", "Z score > ", round(Z_critical,2), ", we will reject the null hypothesis.")
...
ifelse(abs(Z) > Z_critical, 
       "Computed Z score is greater than critical value, we reject the null hypothesis.",
       "Computed Z score is less than or equal to critical value, we fail to reject the null hypothesis.")

# 9. Conclusion -----------------------------------------------------------

print("We can conclude that the mean baseline systolic blood pressure of our data is greater than historical benchmark of 140 mmHg")

# 10. p value -------------------------------------------------------------

p <- pnorm(abs(Z), lower.tail = F)  ; round(p,3)

ifelse(p < alpha,
       "We have strong evidence that the mean baseline systolic blood pressure of our data is significantly greater than historical benchmark of 140 mmHg",
       "We have weak evidence that the mean baseline systolic blood pressure of our data is significantly greater than historical benchmark of 140 mmHg")
